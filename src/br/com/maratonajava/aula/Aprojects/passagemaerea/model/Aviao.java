...
import br.com.maratonajava.aula.Aprojects.passagemaerea.assento.TipoAssento;
...
public class Aviao {
    ...
    public ArrayList<Pessoa> getPassageiros(){//retorna um ArrayList de passageiros com assento ocupado neste avião
        ArrayList<Pessoa> passageiros = new ArrayList<Pessoa>();
        for (int i = 0; i < assentoOrganizado.length ; i++ ){//itera sobre todos os assentos em busca de passageiros
            for(int j = 0; j < assentoOrganizado[i].length ; j++){
                if (assentoOrganizado[i][j] != null && assentoOrganizado[i][j].getPessoa() != null){
                    passageiros.add(assentoOrganizado[i][j].getPessoa());//quando achado adiciona na lista 
                } 
            }
        }
        return passageiros;
